...
import { FhirDehydrator, FhirUtilities } from 'meteor/clinical:hl7-fhir-data-infrastructure';
...
MedicalRecordsExporter = {
  ...
  exportBulkData(filterString, excludeEnteredInError, includeCoverLetter, includeTableOfContents, patient){
    ...
    if(includeCoverLetter){
      ...
      console.log('generated cover page...', newComposition)
      if(filterString){
  ...
  exportContinuityOfCareDoc(filterString, excludeEnteredInError, includeCoverLetter, includeTableOfContents, patient){
    console.log('Exporting a Continuity Of Care Document', filterString, excludeEnteredInError, includeTableOfContents, patient);
    if(typeof filterString !== "string"){
      console.log('filterString.target.value', ilterString.target.value);
    }
    ...
    if(filterString){
      ...
      set(newComposition, 'subject.display', get(patient, 'name[0].text', ''))
      set(newComposition, 'subject.reference', 'Patient/' + get(patient, 'id', ''))

      set(newComposition, 'author[0].display', get(patient, 'name[0].text', ''))
      set(newComposition, 'author[0].reference', 'Patient/' + get(patient, 'id', ''))
