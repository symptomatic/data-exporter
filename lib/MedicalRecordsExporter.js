...
import { Anonymizer } from '@node-on-fhir/admin-tools/lib/Anonymizer';
import { PATIENT_COMPARTMENT_MAP, buildPatientQuery } from '@node-on-fhir/admin-tools/lib/PatientCompartmentMapper';
...
Meteor.startup(async function(){
  ...
  DiagnosticReports = Meteor.Collections.DiagnosticReports;
  ...
  NutritionIntakes = Meteor.Collections.NutritionIntakes;
  ValueSets = Meteor.Collections.ValueSets;
})

//====================================================================================

// const + globalThis: bare-global assignment throws in this strict (import-bearing) file under Rspack/swc
const MedicalRecordsExporter = globalThis.MedicalRecordsExporter = {
  ...
  exportBulkData(filterString, excludeEnteredInError, includeCoverLetter, includeTableOfContents, dataContent, selectedPatientId){
    console.log('Exporting FHIR Bulk Data...', filterString, excludeEnteredInError, dataContent, selectedPatientId);
    ...
    if(toggleStates){
      ...
      // Track which patients have been added to avoid duplicates
      let addedPatientIds = [];
      ...
      if((typeof Patients === "object") && toggleStates.Patient){
        ...
        Patients.find(patientQuery).forEach(function(patient){
          ...
          addedPatientIds.push(patient.id);
          if(newCoverPage){
      ...
      // Add selected patient if requested and not already included
      if(selectedPatientId && !addedPatientIds.includes(selectedPatientId)){
        console.log('Adding currently selected patient to bulk export:', selectedPatientId);
        let selectedPatient = Patients.findOne({id: selectedPatientId});
        if(selectedPatient){
          delete selectedPatient._document;
          fhirEntries.push(selectedPatient)
          addedPatientIds.push(selectedPatient.id);
          if(newCoverPage){
            newCoverPage.section.push({
          ...
          console.warn('Selected patient not found:', selectedPatientId);
        }
      }
      ...
      if((typeof MolecularSequences === "object") && toggleStates.MolecularSequence){
        console.log('Writing ' + MolecularSequences.find().count() + ' MolecularSequence records to export file.');

        let molecularSequencesQuery = {}
        MolecularSequences.find(molecularSequencesQuery).forEach(function(molecularSequence){
          delete molecularSequence._document;
          fhirEntries.push(molecularSequence)
          if(newCoverPage){
            newCoverPage.section.push({
              reference: "MolecularSequence/" + molecularSequence.id
            })
          }
        })
      }
      if((typeof NutritionIntakes === "object") && toggleStates.NutritionIntake){
        console.log('Writing ' + NutritionIntakes.find().count() + ' NutritionIntake records to export file.');

        let nutritionIntakesQuery = {}
        NutritionIntakes.find(nutritionIntakesQuery).forEach(function(nutritionIntake){
          delete nutritionIntake._document;
          fhirEntries.push(nutritionIntake)
          if(newCoverPage){
            newCoverPage.section.push({
              reference: "NutritionIntake/" + nutritionIntake.id
            })
          }
        })
      }
      ...
      // Generic fallback: export any resource types not handled above
      if(Meteor.Collections && toggleStates){
        var handledTypes = [
          'Patient', 'AllergyIntolerance', 'BodyStructure', 'CarePlan', 'CareTeam',
          'Condition', 'Consent', 'Claim', 'Communication', 'CommunicationRequest',
          'Composition', 'Contract', 'ClinicalImpression', 'Device', 'DocumentReference',
          'DocumentManifest', 'DiagnosticReport', 'ExplanationOfBenefit', 'Encounter',
          'Goal', 'Group', 'Immunization', 'ImagingStudy', 'Location', 'List',
          'Measure', 'MeasureReport', 'Medication', 'MedicationAdministration',
          'MedicationOrder', 'MedicationResponse', 'MedicationStatement', 'MessageHeader',
          'Observation', 'Organization', 'Person', 'Practitioner', 'Procedure',
          'Questionnaire', 'QuestionnaireResponse', 'RiskAssessment', 'Sequence',
          'MolecularSequence', 'NutritionIntake',
          'Specimen', 'SearchParameter', 'StructureDefinition', 'ServiceRequest',
          'Task', 'ValueSet'
        ];
        Object.keys(toggleStates).forEach(function(resourceType){
          if(toggleStates[resourceType] && handledTypes.indexOf(resourceType) === -1){
            var pluralName = resourceType + 's';
            var collection = Meteor.Collections[pluralName];
            if(collection && typeof collection.find === 'function'){
              console.log('Writing ' + collection.find().count() + ' ' + resourceType + ' records to export file (generic fallback).');
              collection.find({}).forEach(function(resource){
                delete resource._document;
                fhirEntries.push(resource);
                if(newCoverPage){
                  newCoverPage.section.push({
    ...
    if(Array.isArray(fhirEntries)){
      // Filter by individually selected resource IDs if available
      let selectedResourceIds = Session.get('exportSelectedResourceIds') || {};
      let hasSelectedIds = Object.keys(selectedResourceIds).length > 0;
      if(hasSelectedIds){
        fhirEntries = fhirEntries.filter(function(entry){
          let entryId = get(entry, '_id') || get(entry, 'id');
          return selectedResourceIds[entryId];
        });
      }

      console.log('Adding ' + fhirEntries.length + ' resource entries to export file.');
  ...
  exportContinuityOfCareDoc(patientId, excludeEnteredInError, includeCoverLetter, includeTableOfContents, includePatientSummary, patient, selectedPatientId){
    console.log('Exporting a Continuity Of Care Document', patientId, excludeEnteredInError, includeTableOfContents, includePatientSummary, patient, selectedPatientId);
    ...
    if(typeof includePatientSummary === "object"){
      ...
      if(toggleStates){
        ...
        // Add selected patient if requested and not already included
        if(selectedPatientId && !addedPatientIds.includes(selectedPatientId)){
          console.log('Adding currently selected patient to export:', selectedPatientId);
          let selectedPatient = Patients.findOne({id: selectedPatientId});
          if(selectedPatient){
            delete selectedPatient._document;
            fhirEntries.push({
              fullUrl: "Patient/" + selectedPatient.id,
              resource: selectedPatient,
              request: {
                method: "PUT",
                url: "Patient/" + selectedPatient.id
              }
            })
            addedPatientIds.push(selectedPatient.id);
            newCoverPage.resource.section.push({
        ...
        if((typeof Goals === "object") && toggleStates.Goal){
          let goalsQuery = {}
          Goals.find(goalsQuery).forEach(function(goal){
            delete goal._document;
            fhirEntries.push({
        ...
        if((typeof MedicationRequests === "object") && toggleStates.MedicationRequest){
          let medicationRequestsQuery = {}
          MedicationRequests.find(medicationRequestsQuery).forEach(function(medicationRequest){
            delete medicationRequest._document;
            fhirEntries.push({
              fullUrl: "MedicationRequest/" + medicationRequest.id,
        ...
        if((typeof MolecularSequences === "object") && toggleStates.MolecularSequence){
          ...
          MolecularSequences.find(molecularSequencesQuery).forEach(function(molecularSequence){
            delete molecularSequence._document;
            fhirEntries.push({
              fullUrl: "MolecularSequence/" + molecularSequence.id,
              resource: molecularSequence,
              request: {
                method: "PUT",
                url: "MolecularSequence/" + molecularSequence.id
              }
            })
            newCoverPage.resource.section.push({
              reference: "MolecularSequence/" + molecularSequence.id
        ...
        if((typeof NutritionIntakes === "object") && toggleStates.NutritionIntake){
          ...
          NutritionIntakes.find(nutritionIntakesQuery).forEach(function(nutritionIntake){
            delete nutritionIntake._document;
            fhirEntries.push({
              fullUrl: "NutritionIntake/" + nutritionIntake.id,
              resource: nutritionIntake,
              request: {
                method: "PUT",
                url: "NutritionIntake/" + nutritionIntake.id
              }
            })
            newCoverPage.resource.section.push({
              reference: "NutritionIntake/" + nutritionIntake.id
      ...
      if(Array.isArray(fhirEntries)){
        ...
        if(hasSelectedIds){
          fhirEntries = fhirEntries.filter(function(entry){
            let entryId = get(entry, 'resource._id') || get(entry, 'resource.id') || get(entry, '_id') || get(entry, 'id');
            return selectedResourceIds[entryId];
          });
        }
...
/**
 * Collects all resources in the Patient Compartment for a given patient.
 * Server-side only: iterates PATIENT_COMPARTMENT_MAP, fetches patient + all
 * linked resources via findAsync(). Returns plain array of FHIR resources.
 */
MedicalRecordsExporter.collectPatientCompartment = async function(patientId) {
  const collections = global.Collections || Meteor.Collections || {};
  const Patients = collections['Patients'];

  if (!Patients) {
    throw new Error('Patients collection not registered');
  }

  const patient = await Patients.findOneAsync({ _id: patientId });
  if (!patient) {
    throw new Error('Patient not found with _id: ' + patientId);
  }

  const mongoId = get(patient, '_id');
  const fhirId = get(patient, 'id');

  // Start with the Patient resource
  const resources = [JSON.parse(JSON.stringify(patient))];
  ...
  for (const [collectionName, refField] of Object.entries(PATIENT_COMPARTMENT_MAP)) {
    ...
    try {
      ...
      for (let i = 0; i < docs.length; i++) {
        resources.push(JSON.parse(JSON.stringify(docs[i])));
      }
    } catch (error) {
      console.warn('[MedicalRecordsExporter.collectPatientCompartment] Error fetching ' + collectionName + ':', error.message);
    }
  }

  console.log('[MedicalRecordsExporter.collectPatientCompartment] Collected ' + resources.length + ' resources for patient ' + patientId);
  return resources;
};

/**
 * Writes modified resources back to their respective collections via updateAsync().
 * Uses resourceType to determine collection name (pluralizes).
 * Server-side only.
 */
MedicalRecordsExporter.writeBackResources = async function(resources) {
  const collections = global.Collections || Meteor.Collections || {};
  let totalUpdated = 0;
  const errors = [];
  ...
  for (let i = 0; i < resources.length; i++) {
    ...
    if (!resourceType || !resourceId) {
      errors.push('Resource at index ' + i + ' missing resourceType or _id');
      continue;
    }

    // Pluralize resource type to get collection name
    ...
    if (!collection || typeof collection.updateAsync !== 'function') {
      errors.push('Collection not found for ' + resourceType + ' (' + collectionName + ')');
      continue;
    }

    try {
      // Remove _id from the $set payload (can't update _id)
      ...
      errors.push(resourceType + '/' + resourceId + ': ' + error.message);
    }
  }

  console.log('[MedicalRecordsExporter.writeBackResources] Updated ' + totalUpdated + ' resources, ' + errors.length + ' errors');
  return { totalUpdated: totalUpdated, errors: errors };
};

/**
 * Orchestrator: collectPatientCompartment → Anonymizer.anonymize → returns result.
 * Does NOT write back (caller decides whether to write or just export).
 */
MedicalRecordsExporter.exportAnonymized = async function(patientId, options) {
  const resources = await MedicalRecordsExporter.collectPatientCompartment(patientId);
  const result = Anonymizer.anonymize(resources, options);

  console.log('[MedicalRecordsExporter.exportAnonymized] Anonymized ' + result.resources.length + ' resources, ' + result.warnings.length + ' warnings');
  return result;
};

/**
 * Same as exportAnonymized but wraps result in a FHIR Bundle (transaction format).
 */
MedicalRecordsExporter.exportAnonymizedBundle = async function(patientId, options) {
  const result = await MedicalRecordsExporter.exportAnonymized(patientId, options);

  const bundleEntries = result.resources.map(function(resource) {
  ...
  console.log('[MedicalRecordsExporter.exportAnonymizedBundle] Generated bundle with ' + bundleEntries.length + ' entries');
  return {
    bundle: bundle,
    idMap: result.idMap,
